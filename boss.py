...
def get_proxy():#芝麻代理获取单个代理
    time.sleep(2)#芝麻代理每次请求有1s-2s间隙
    url = "http://webapi.http.zhimacangku.com/getip?num=1&type=2&pro=&city=0&yys=0&port=11&time=1&ts=1&ys=0&cs=1&lb=1&sb=0&pb=45&mr=1&regions="
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/73.0.3683.103 Safari/537.36"
    }
    ...
    first_proxy = str(ip)+":"+str(port)
    proxy = {
        "http":"http://"+first_proxy,
        "https":"https://"+first_proxy
    }
    return proxy

def parse_page(url):#解析各页
    works = []
    proxy = get_proxy()
    while True:#被反爬捕捉时更换代理重新请求
        try:
    ...
    for li in lis:
        ...
        result = parse_detail(detail_url,works,proxy)
        proxy = result[1]#传递代理供下次循环使用
        works = result[0]#传递职位列表
    save(works)
    next_href = html.xpath("//div[@class='page']/a[last()]/@href")
    print(next_href)
    if next_href !=[] or next_href != ['javascript:;']:
        next_href = "https://www.zhipin.com"+next_href[0]
        parse_page(next_href)
    else:
        pass

def parse_detail(url,works,proxy):#职位详情页
    work_info = {}
    work_info["工作详情页"] = url
    while True:#被反爬捕捉时更换代理重新请求
        try:
    ...
    return works,proxy#代理可用时传给下一次循环继续使用
